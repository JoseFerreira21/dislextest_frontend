...
export class TestSeleccionarPalabraComponent implements OnInit {
  ...
  async ngOnInit() {
    try {
        ...
        this.cargarImagenes();
        this.textToSpeechService.speak('Selecciona la palabra escrita correctamente');
    } catch (error) {
  ...
  verificarSeleccionCompleta() {
    this.seleccionCompleta = this.grupos.every((grupo, index) => this.seleccion[index] !== undefined);
