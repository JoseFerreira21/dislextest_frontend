...
@Component({
  selector: 'app-test-encerrar-silaba-cf',
  templateUrl: './test-encerrar-silaba-cf.component.html',
  styleUrls: ['./test-encerrar-silaba-cf.component.scss']
})
export class TestEncerrarSilabaCfComponent {

  ejercicios: EncerrarSilabaConcienciaFonologica[] = [];
  ...
  async ngOnInit() {
    try {
      ...
      await Promise.all([this.cargarAreas(), this.cargarEncerrarSilabaConcienciaFonologica()]);
  ...
  escucharInstruccion() {
    this.textToSpeechService.speak('Encierra la sílaba que falta, dando click en ella');
  }
  ...
  async cargarEncerrarSilabaConcienciaFonologica(): Promise<void> {
    return new Promise<void>((resolve, reject) => {
      this.testService.getEncerrarSilabasConsienciaFonologica().subscribe({
        next: (data) => {
  ...
  verificarSeleccionCompleta() {
    ...
    if (Object.keys(this.respuestasSeleccionadas).length === this.ejercicios.length) {
  ...
  public guardar = async (testId: number) => {
    ...
    for (let index = 0; index < this.ejercicios.length; index++) {
      ...
      element.validez = this.respuestasSeleccionadas[index] === element.respuesta
